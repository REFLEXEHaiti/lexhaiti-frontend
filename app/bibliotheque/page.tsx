...
const COLLECTIONS_BASE = [
  {
    id: 'codes', titre: 'Codes Haïtiens', icone: '📚', couleur: BORDEAUX,
    description: 'Versions annotées et mises à jour des principaux codes de droit haïtien',
    items: [
      { id: 'c1', titre: 'Code Civil Haïtien', pages: 312, annee: '1826 (rev. 2006)', gratuit: true, type: 'document' },
      { id: 'c2', titre: 'Code Pénal Haïtien', pages: 187, annee: '1835 (rev. 2020)', gratuit: true, type: 'document' },
      { id: 'c3', titre: 'Code de Procédure Civile', pages: 428, annee: '1825 (rev. 2015)', gratuit: false, type: 'document' },
      { id: 'c4', titre: 'Code du Travail', pages: 198, annee: '1984 (rev. 2021)', gratuit: true, type: 'document' },
    ],
  },
  {
    id: 'constitutions', titre: 'Constitutions & Lois', icone: '🏛️', couleur: '#1A3A6B',
    description: 'Textes constitutionnels historiques et commentaires doctrinaux',
    items: [
      { id: 'co1', titre: 'Constitution de 1987 (amendée 2012)', pages: 98, annee: '1987', gratuit: true, type: 'document' },
      { id: 'co2', titre: 'Constitutions historiques 1801-1983', pages: 420, annee: 'Recueil', gratuit: false, type: 'document' },
      { id: 'co3', titre: 'Acte de l\'Indépendance 1804', pages: 12, annee: '1804', gratuit: true, type: 'document' },
    ],
  },
  {
    id: 'jurisprudences', titre: 'Jurisprudences', icone: '⚖️', couleur: '#065F46',
    description: 'Décisions de la Cour de Cassation, tribunaux civils et cours d\'appel',
    items: [
      { id: 'j1', titre: 'Arrêts Cour de Cassation 2020-2026', pages: 890, annee: '2020-2026', gratuit: false, type: 'document' },
      { id: 'j2', titre: 'Jurisprudences droit foncier 2015-2024', pages: 445, annee: '2015-2024', gratuit: false, type: 'document' },
    ],
  },
  {
    id: 'doctrine', titre: 'Doctrine & Articles', icone: '✍️', couleur: '#7C2D12',
    description: 'Articles doctrinaux, thèses et commentaires de juristes haïtiens',
    items: [
      { id: 'd1', titre: 'La procédure pénale haïtienne expliquée', pages: 345, annee: '2022', gratuit: true, type: 'document' },
      { id: 'd2', titre: 'Introduction au droit haïtien', pages: 198, annee: '2020', gratuit: true, type: 'document' },
      { id: 'd3', titre: 'Droit foncier haïtien — Traité complet', pages: 567, annee: '2023', gratuit: false, type: 'document' },
    ],
  },
  {
    id: 'formulaires', titre: 'Formulaires & Modèles', icone: '📝', couleur: '#4C1D95',
    description: 'Actes types, formulaires officiels et modèles de plaidoiries',
    items: [
      { id: 'f1', titre: 'Formulaires Tribunal Civil PAP', pages: 78, annee: '2024', gratuit: true, type: 'document' },
      { id: 'f2', titre: 'Modèles de plaidoiries commentés', pages: 156, annee: '2023', gratuit: false, type: 'document' },
    ],
  },
  {
    id: 'videos', titre: 'Vidéos & Broadcasts', icone: '🎬', couleur: '#C2410C',
    description: 'Conférences juridiques, broadcasts archivés et cours vidéo',
    items: [],
  },
];
      ...
      <section style={{ background: '#1A0000', padding: 'clamp(40px,6vw,80px) clamp(20px,5vw,48px)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', textAlign: 'center' }}>
          ...
          <p style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 'clamp(13px,1.6vw,17px)', color: 'rgba(255,255,255,0.65)', maxWidth: 560, margin: '0 auto 32px', lineHeight: 1.7 }}>
            Codes annotés, jurisprudences, doctrine, formulaires et vidéos juridiques.
          </p>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: 'clamp(24px,4vw,48px) clamp(20px,5vw,48px)', display: 'grid', gridTemplateColumns: '240px 1fr', gap: 32, alignItems: 'start' }}>
          ...
          <div style={{ background: 'white', border: '1px solid #E8E4DC', borderRadius: 10, overflow: 'hidden', position: 'sticky', top: 72 }}>
            ...
            {collections.map(col => (
              <button key={col.id} onClick={() => setCategorieActive(col.id)}
                style={{ width: '100%', padding: '12px 16px', background: categorieActive === col.id ? `${BORDEAUX}08` : 'white', border: 'none', borderLeft: `3px solid ${categorieActive === col.id ? BORDEAUX : 'transparent'}`, cursor: 'pointer', textAlign: 'left', display: 'flex', gap: 10, alignItems: 'center' }}>
        ...
        <main>
            ...
            {estAdmin && (
              <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                ...
                <button onClick={ouvrirModal}
                  style={{ padding: '10px 20px', background: BORDEAUX, color: 'white', border: 'none', borderRadius: 8, fontFamily: "'Helvetica Neue',Arial,sans-serif", fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
                  + Ajouter
                </button>
              </div>
            )}
                ...
                <div key={item.id ?? i} style={{ background: 'white', border: '1px solid #E8E4DC', borderRadius: 10, padding: '18px 22px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, flexWrap: 'wrap' }}>
                      ...
                      {item.gratuit && <span style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 10, color: '#166534', background: '#DCFCE7', padding: '2px 8px', borderRadius: 100, fontWeight: 700 }}>GRATUIT</span>}
                      ...
                      {estVideo(item) && item.videoUrl?.startsWith('data:') && <span style={{ fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 10, color: BORDEAUX, background: `${BORDEAUX}10`, padding: '2px 8px', borderRadius: 100, fontWeight: 700 }}>📁 Fichier</span>}
                  ...
                  <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                    {item.gratuit || estAdmin ? (
                      estVideo(item) ? (
                        <button onClick={() => setLecteur(item)} style={{ padding: '8px 16px', background: BORDEAUX, color: 'white', border: 'none', borderRadius: 6, fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>▶ Regarder</button>
                      ) : (item.fichierUrl ? (
                        <button onClick={() => setLecteur(item)} style={{ padding: '8px 16px', background: BORDEAUX, color: 'white', border: 'none', borderRadius: 6, fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>📖 Lire</button>
                      ) : (
                        <button style={{ padding: '8px 16px', background: BORDEAUX, color: 'white', border: 'none', borderRadius: 6, fontFamily: "'Helvetica Neue',Arial,sans-serif", fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>Lire →</button>
                      ))
                    ) : (
          ...
          <div style={{ marginTop: 32, background: `linear-gradient(135deg, ${BORDEAUX}10, ${OR}10)`, border: `1px solid ${BORDEAUX}25`, borderRadius: 12, padding: '24px 28px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
            ...
            <Link href="/premium" style={{ padding: '12px 24px', background: BORDEAUX, color: 'white', borderRadius: 6, textDecoration: 'none', fontFamily: "'Helvetica Neue',Arial,sans-serif", fontWeight: 700, fontSize: 13, flexShrink: 0 }}>Voir les plans →</Link>
          </div>
        </main>
      </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ background: 'white', borderRadius: 14, padding: '28px 32px', width: '100%', maxWidth: 560, boxShadow: '0 24px 64px rgba(0,0,0,0.2)', maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 22, paddingBottom: 14, borderBottom: `2px solid ${BORDEAUX}` }}>
              <h2 style={{ fontFamily: 'Georgia,serif', fontSize: 18, color: BORDEAUX, margin: 0 }}>Ajouter une ressource</h2>
            ...
            <form onSubmit={sauvegarder} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 14 }}>
                  {[
                    { val: 'DOCUMENT', label: '📄 Document', desc: 'PDF, texte juridique' },
                    { val: 'VIDEO', label: '🎬 Vidéo', desc: 'Cours, conférence' },
                  ].map(opt => (
                    <div key={opt.val} onClick={() => setForm((p: any) => ({ ...p, typeContenu: opt.val }))}
                      style={{ border: `2px solid ${form.typeContenu === opt.val ? BORDEAUX : '#E2E8F0'}`, borderRadius: 10, padding: '12px', cursor: 'pointer', background: form.typeContenu === opt.val ? `${BORDEAUX}06` : 'white', textAlign: 'center' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
                      {[
                        ...
                        <div key={opt.val} onClick={() => setForm((p: any) => ({ ...p, sourceDoc: opt.val }))}
                          style={{ border: `2px solid ${form.sourceDoc === opt.val ? BORDEAUX : '#E2E8F0'}`, borderRadius: 10, padding: '10px', cursor: 'pointer', background: form.sourceDoc === opt.val ? `${BORDEAUX}06` : 'white', textAlign: 'center' }}>
                    ...
                    {form.sourceDoc === 'PDF_UPLOAD' && (
                      <div>
                        ...
                        <div onClick={() => pdfRef.current?.click()}
                          style={{ border: `2px dashed ${pdfNom ? BORDEAUX : '#E2E8F0'}`, borderRadius: 10, padding: '20px', textAlign: 'center', cursor: 'pointer', background: pdfNom ? `${BORDEAUX}04` : '#F8FAFC' }}>
                          {pdfNom ? (
                            <div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
                      {[
                        ...
                        <div key={opt.val} onClick={() => setForm((p: any) => ({ ...p, sourceVideo: opt.val }))}
                          style={{ border: `2px solid ${form.sourceVideo === opt.val ? BORDEAUX : '#E2E8F0'}`, borderRadius: 10, padding: '10px', cursor: 'pointer', background: form.sourceVideo === opt.val ? `${BORDEAUX}06` : 'white', textAlign: 'center' }}>
